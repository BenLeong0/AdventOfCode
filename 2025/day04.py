...
part2ol = lambda filename: [
    ...
    (w := len(rows[0])),
    (h := len(rows)),
    (rd := {(x, y): rows[x][y] == "@" for (x, y) in product(range(w), range(h))}),
    (
        n := lambda i, j, r: (
            not r[i, j]
            or len(
                [
                    (x, y)
                    for (x, y) in product(range(i - 1, i + 2), range(j - 1, j + 2))
                    if (x != i or y != j) and 0 <= x < w and 0 <= y < h and r[x, y]
                ]
            )
        )
    ),
    (
        ...
        - sum(
            reduce(
                lambda prev, next: {
                    (x, y): v and n(x, y, prev) >= 4 for ((x, y), v) in prev.items()
                },
                range(100),
                rd,
            ).values()
